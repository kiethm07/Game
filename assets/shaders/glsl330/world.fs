...
uniform sampler2D shadowMap1;        ///< far cascade, static over the arena

// NOTE: everything from here to the end of shadowFactor() is DUPLICATED
// VERBATIM in the other shader (world.fs <-> skinning.fs). GLSL 330 has no
// #include, so the two copies have to be edited together -- if they drift,
// characters and the ground they stand on will disagree about where the
// shadows are.
...
float shadowFactor(vec3 worldPos, vec3 normal, vec3 lightVec)
{
    ...
    else
    {
        ...
        // Outside the far cascade there is no depth to compare against. It
        // covers 48m but the ground plane is 200m, so this is the normal case
        // out towards the horizon: treat it as lit, never as shadowed.
        float farShadow = 0.0;
        if (p1.z <= 1.0 &&
